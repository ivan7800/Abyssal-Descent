...
function setBadge(){const badge=document.getElementById('badge');if(badge){badge.textContent='FINAL · v1.4.1';badge.classList.toggle('v13-badge-daily',isDailySeed(api.getState?.().seed))}}
...
setBadge()
renderHome();syncProfileBack();setBadge()
window.__abyssalV14={showHome:()=>setHero(true),newExpedition:()=>setHero(false),showAct:()=>showAct(api.getState()),showBoss:()=>showBoss(api.getState()),route:openFromHome,version:'1.4.1'}
