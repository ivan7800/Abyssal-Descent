const VERSION='abyssal-descent-v1.4.1-r1'
const CORE=[
...
self.addEventListener('fetch',event=>{
  ...
  if(event.request.mode==='navigate'){
    event.respondWith(fetch(event.request).then(response=>{if(response.ok){const copy=response.clone();caches.open(VERSION).then(c=>c.put('./index.html',copy))}return response}).catch(()=>caches.match('./index.html')))
    return
  }
