...
const key = (p) => `${p.x},${p.y}`;
export function generateDungeon(seed, width = 19, height = 15, floor = 1) {
    if (width % 2 === 0 || height % 2 === 0)
        throw new Error('Dungeon dimensions must be odd.');
    ...
    const used = new Set([key(start), key(exit)]);
    const takeByBand = (minRatio, maxRatio, count) => {
...
export function bfsDistances(grid, start) {
    ...
    for (let i = 0; i < queue.length; i++) {
        ...
        for (const s of steps) {
            ...
            const nk = key(n);
            const cell = typeof grid[n.y] === 'string' ? grid[n.y]?.[n.x] : grid[n.y]?.[n.x];
            if (cell !== '.' || result.has(nk))
                continue;
            result.set(nk, d + 1);
            queue.push(n);
        }
