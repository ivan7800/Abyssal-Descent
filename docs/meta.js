...
export function makeExportBundle(codex,language){
 ...
 try{const save=JSON.parse(raw);if(!validSaveEnvelope(save))return {ok:false,reason:'invalid'};return {ok:true,bundle:{format:'abyssal-descent-save',exportVersion:1,gameVersion:'1.2.0',exportedAt:new Date().toISOString(),language:language==='es'?'es':'en',save,codex:sanitizeCodex(codex)}}}catch{return {ok:false,reason:'invalid'}}
}
