...
function syncProfileBack(){
 ...
 if(api.getState?.().mode==='title'&&!heroVisible){if(!btn){btn=document.createElement('button');btn.id='v14-back';btn.className='v14-back';btn.dataset.v14='home';panel.prepend(btn)}const label=`← ${t('back')}`;if(btn.textContent!==label)btn.textContent=label;panel.classList.add('v14-profile-open')}
...
document.addEventListener('click',event=>{const b=event.target.closest?.('[data-v14]');if(b){event.preventDefault();openFromHome(b.dataset.v14);return}if(event.target.closest?.('[data-lang],[data-archetype]'))setTimeout(()=>{renderHome();syncProfileBack();setBadge()},0)})
