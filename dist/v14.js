...
function setHero(show){heroVisible=!!show;renderHome();syncProfileBack()}
function renderHome(){
...
function maybeDailyResult(snapshot){
 ...
 const score=calculateDailyScore(snapshot),r=snapshot.runStats??{},best=todayBest(),items=[[t('score'),score],[t('steps'),snapshot.steps||0],[t('damage'),r.damageTaken||0],[t('secrets'),snapshot.secretCount||0]]
 const stats=items.map(([a,b])=>`<div><span>${esc(a)}</span><strong>${esc(b)}</strong></div>`).join('')
 setTimeout(()=>showStage(best===score?t('newBest'):t('dailyResult'),tr(snapshot.endingTitle),best&&best!==score?`${t('previousBest')}: ${best}`:'',stats),450)
}
...
function openFromHome(action){if(action==='resume'){api.send('load');setHero(false)}else if(action==='new')setHero(false);else if(action==='daily'){v13.openDaily?.();}else if(action==='codex'){v12.openCodex?.()}else if(action==='achievements'){v13.openAchievements?.()}else if(action==='settings'){v13.openSettings?.()}else if(action==='home')setHero(true)}
...
const badge=document.getElementById('badge');if(badge)badge.textContent='FINAL · v1.4.0'
renderHome();syncProfileBack()
window.__abyssalV14={showHome:()=>setHero(true),newExpedition:()=>setHero(false),showAct:()=>showAct(api.getState()),showBoss:()=>showBoss(api.getState()),version:'1.4.0'}
