...
import { bfsDistances, generateDungeon, seededRandom } from './generator.js';
import { ARCHETYPES, ENEMY_DEFS, FLOOR_DATA, ITEM_DEFS, SETPIECE_TEXT } from './content.js';
import { AbyssAudio } from './audio.js';
...
const storageSet = (storageKey, value) => {
    try {
...
const storageRemove = (storageKey) => {
    try {
        if (typeof localStorage !== 'undefined')
...
const isPoint = (value) => isRecord(value) && isInteger(value.x) && isInteger(value.y);
const isLayout = (value) => {
    if (!isRecord(value) || !Array.isArray(value.map) || !value.map.every(row => typeof row === 'string' && row.length > 0))
        return false;
    const width = value.map[0]?.length ?? 0;
    if (!width || !value.map.every(row => row.length === width && /^[.#]+$/.test(row)))
        return false;
    const pointArray = (candidate) => Array.isArray(candidate) && candidate.every(isPoint);
...
const isRunStats = (value) => isRecord(value)
    && ['kills', 'bossesDefeated', 'elitesDefeated', 'damageDealt', 'damageTaken', 'itemsUsed', 'retreats', 'rests'].every(k => isFiniteNumber(value[k]) && value[k] >= 0);
const isInventory = (value) => Array.isArray(value) && value.every(item => isRecord(item)
...
const isEnemyArray = (value) => Array.isArray(value) && value.every(enemy => isRecord(enemy)
...
const isLootArray = (value) => Array.isArray(value) && value.every(loot => isRecord(loot)
    && isInteger(loot.x) && isInteger(loot.y) && typeof loot.defId === 'string' && loot.defId in ITEM_DEFS && typeof loot.taken === 'boolean');
const isSaveData = (value) => {
    if (!isRecord(value) || value.version !== 5 || !['playing', 'ending'].includes(String(value.mode)))
        return false;
    if (typeof value.seed !== 'string' || value.seed.length > 48 || typeof value.archetype !== 'string' || !(value.archetype in ARCHETYPES))
        return false;
    if (!isInteger(value.floor) || value.floor < 1 || value.floor > 4)
        return false;
    if (!isInteger(value.level) || value.level < 1 || !isFiniteNumber(value.xp))
        return false;
    if (!isLayout(value.layout) || !isRecord(value.player) || !isInteger(value.player.x) || !isInteger(value.player.y)
        || !isInteger(value.player.dir) || value.player.dir < 0 || value.player.dir > 3)
        return false;
    if (![value.hp, value.maxHp, value.baseMaxSanity, value.sanity, value.steps, value.itemSerial, value.turnSerial, value.abilityCooldown, value.secretCount].every(isFiniteNumber))
        return false;
    if (value.maxHp <= 0 || value.baseMaxSanity <= 0 || value.hp < 0 || value.hp > value.maxHp
        || value.sanity < 0 || value.steps < 0 || value.itemSerial < 0 || value.turnSerial < 0 || value.abilityCooldown < 0 || value.secretCount < 0)
        return false;
    if (!isStringArray(value.log) || !isStringArray(value.journal) || !isInventory(value.inventory) || !isEnemyArray(value.enemies) || !isLootArray(value.loot))
        return false;
    if (![value.triggeredEvents, value.questFlags, value.discovered, value.setpieceTriggered].every(isStringArray))
        return false;
    if (!Array.isArray(value.sanctuaryUsed) || !value.sanctuaryUsed.every(isInteger))
        return false;
    if (!(value.activeEnemyId === null || typeof value.activeEnemyId === 'string') || !(value.equippedWeaponId === null || typeof value.equippedWeaponId === 'string')
        || !(value.equippedCharmId === null || typeof value.equippedCharmId === 'string'))
        return false;
    if (value.activeEnemyId !== null && !value.enemies.some(enemy => enemy.id === value.activeEnemyId && !enemy.defeated))
        return false;
    if (value.equippedWeaponId !== null && !value.inventory.some(item => item.instanceId === value.equippedWeaponId && item.kind === 'weapon'))
        return false;
    if (value.equippedCharmId !== null && !value.inventory.some(item => item.instanceId === value.equippedCharmId && item.kind === 'charm'))
        return false;
    if (![value.guarded, value.counterReady, value.audioEnabled, value.reducedMotion].every(v => typeof v === 'boolean'))
        return false;
    if (typeof value.endingTitle !== 'string' || typeof value.endingText !== 'string' || !isRunStats(value.runStats))
        return false;
    return true;
};
const EVENT_TEXT = [
...
export class DungeonScene extends Phaser.Scene {
    ...
    sanity = 1;
    steps = 0;
    logLines = ['Choose an expedition profile to begin.'];
    journalEntries = [];
    inventory = [];
    ...
    endingTitle = '';
    endingText = '';
    setpieceTriggered = new Set();
    secretCount = 0;
    audioEnabled = true;
    reducedMotion = false;
    runStats = freshRunStats();
    audio = new AbyssAudio();
    ...
    loadPreferences() {
        try {
            ...
            storageRemove(SETTINGS_KEY); /* Discard malformed preferences and keep safe defaults. */
        }
    }
    savePreferences() {
    ...
    onAction(action) {
        ...
        if (action === 'toggle-audio') {
            ...
            this.audio.setEnabled(this.audioEnabled);
            if (this.audioEnabled && this.mode !== 'title')
                this.audio.startDrone(this.floor);
        ...
        if (action === 'toggle-motion') {
            ...
            this.savePreferences();
            this.log(`Reduced motion ${this.reducedMotion ? 'enabled' : 'disabled'}.`);
            this.draw();
            return;
        }
        if (action === 'save') {
        ...
        if (action.startsWith('equip:')) {
            this.equip(action.slice(6));
            return;
        }
        if (action.startsWith('use:')) {
            this.useItem(action.slice(4));
            return;
        }
        if (action.startsWith('ending:')) {
            this.chooseEnding(action.slice(7));
            return;
        }
    ...
    newCampaign(archetype, seed) {
        ...
        if (a.charm)
            this.equippedCharmId = this.addItem(a.charm).instanceId;
        this.sanity = this.maxSanity();
        this.addItem('field_dressing');
        this.addItem('lucid_tonic');
        this.setupFloor(1, true);
    }
    setupFloor(floor, first = false) {
        this.floor = floor;
        ...
        this.populateFloor();
        const data = FLOOR_DATA[this.floor];
        if (!first)
            this.logLines = [...this.logLines, `You descend into ${data.name}.`].slice(-60);
    ...
    makeEnemy(id, kind, x, y, boss, elite = false) {
    ...
    activeEnemy() { return this.enemies.find(e => e.id === this.activeEnemyId && !e.defeated) ?? null; }
    turn(delta) {
    ...
    checkTile(logQuiet) {
        ...
        if (enemy) {
            ...
            if (!this.questFlags.has('tutorial-combat')) {
                this.questFlags.add('tutorial-combat');
                this.log('COMBAT — Attack for damage, Guard against telegraphed danger, Focus to recover sanity, or use your class ability when ready.');
            }
        ...
        if (loot) {
            ...
            if (!this.questFlags.has('tutorial-inventory')) {
                this.questFlags.add('tutorial-inventory');
                this.log('INVENTORY — Equipment can be changed outside combat. Consumables can be used in combat, but doing so costs the turn.');
            }
        ...
        if (setpieceIndex >= 0 && !this.setpieceTriggered.has(setpieceFlag)) {
            ...
            if (piece) {
                ...
                if (piece.reward) {
                    const reward = this.addItem(piece.reward);
                    this.log(`Hidden cache: ${reward.name}.`);
                }
    ...
    interact() {
        ...
        if (this.player.x === this.layout.exit.x && this.player.y === this.layout.exit.y) {
            if (!this.floorObjectiveReady()) {
                this.log(this.floorBlockedMessage());
                this.draw();
                return;
            }
    ...
    floorObjectiveReady() {
        if (this.floor === 1)
            return this.questFlags.has('salt-seal');
        const required = this.floor === 4 ? 3 : 2;
        return Array.from({ length: required }, (_, i) => i).every(i => this.questFlags.has(`altar-${this.floor}-${i}`));
    }
    floorBlockedMessage() {
        if (this.floor === 1)
            return 'The stair is sealed by a rib-shaped lock. The Salt Warden carries what fits it.';
        if (this.floor === 2)
            return 'The gate has two silent resonators. Both hymn altars must be awake.';
        if (this.floor === 3)
            return 'The Heart remains out of phase. Two black-star memories are missing.';
        return 'The final chamber has no stable location. All three anchor sigils must be fixed.';
    }
    startBoss() {
        const kind = FLOOR_DATA[this.floor].boss;
    ...
    enemyDefense(enemy) {
        ...
        if (enemy.kind === 'warden' && (enemy.combatTurn + 1) % 3 === 0)
            defense += 2;
        return defense;
    }
    attack() {
    ...
    useAbility() {
        const enemy = this.activeEnemy();
        if (!enemy || !this.archetype)
            return;
        if (this.abilityCooldown > 0) {
            this.log(`${ARCHETYPES[this.archetype].abilityName} is not ready.`);
            return;
        }
    ...
    startPlayerTurn(tickCooldown = true) {
        this.guarded = false;
        if (tickCooldown && this.abilityCooldown > 0)
            this.abilityCooldown--;
    }
    afterPlayerAttack(enemy) {
        if (enemy.hp <= 0) {
    ...
    enemyTurn(enemy) {
        ...
        if (heavyBossTurn)
            damage += this.floor + phase + 1;
        if (rushingHusk)
            damage += 2;
        if (tollingBell)
            damage += 2;
        if (enemy.nemesisRank)
            damage += Math.min(3, enemy.nemesisRank);
        ...
        if (this.guarded && telegraphedHeavy)
            this.log('You read the tell and absorb the worst of the impact.');
        ...
        if (enemy.kind === 'oracle')
            sanityChance += .12;
        if (enemy.kind === 'mirror_stalker' && enemy.combatTurn % 2 === 0)
            sanityChance += .15;
    ...
    defeatEnemy(enemy) {
        ...
        if (killCharm?.healOnKill) {
            const before = this.hp;
            this.hp = Math.min(this.maxHp, this.hp + killCharm.healOnKill);
            if (this.hp > before)
                this.log(`${killCharm.name} drinks the aftermath. Vitality +${this.hp - before}.`);
        }
        ...
        if (enemy.boss) {
            ...
            if (this.floor === 1) {
                this.addItem('coral_blade');
                this.addJournalOnce('THE SALT KNIGHT — Beneath its mineral armor was a human survey harness, older than the observatory by centuries.');
            }
            if (this.floor === 2) {
                this.addItem('bell_hammer');
                this.addJournalOnce('CHOIR BENEATH — The voices were not singing together. They were arguing about which version of the city should survive.');
            }
            if (this.floor === 3) {
                this.addItem('void_lance');
                this.addJournalOnce('HEART OF THE ABYSS — The Heart was never the source. It was a relay pointed toward something farther down.');
            }
            if (this.floor === 4)
                this.addJournalOnce('THE VOID SAINT — The final guardian was not defending the abyss from you. It was defending you from the choice beyond it.');
    ...
    nextXp() { return 18 + this.level * 16; }
    flee() {
        const enemy = this.activeEnemy();
        if (!enemy || enemy.boss)
            return;
    ...
    addItem(defId) {
        ...
        const item = { ...def, instanceId: `item-${++this.itemSerial}`, ...(def.kind === 'consumable' ? { quantity: 1 } : {}) };
        this.inventory.push(item);
        return item;
    }
    equip(instanceId) {
        const item = this.inventory.find(i => i.instanceId === instanceId);
        if (!item || item.kind === 'consumable' || item.kind === 'quest' || this.hp <= 0 || this.mode !== 'playing')
            return;
        if (this.activeEnemy()) {
            this.log('You cannot reorganize equipment while something is trying to kill you.');
            return;
        }
        if (item.kind === 'weapon')
            this.equippedWeaponId = item.instanceId;
        if (item.kind === 'charm')
            this.equippedCharmId = item.instanceId;
    ...
    useItem(instanceId) {
        ...
        if (index < 0 || this.hp <= 0 || this.mode !== 'playing')
            return;
        const item = this.inventory[index];
        if (item.kind !== 'consumable')
            return;
        if (!this.consumableWouldHelp(item)) {
            this.log(`${item.name} would provide no benefit right now, so you keep it.`);
            return;
        }
        const enemy = this.activeEnemy();
        if (enemy)
        ...
        if (item.restoreSanity) {
            const before = this.sanity;
            this.sanity = Phaser.Math.Clamp(this.sanity + item.restoreSanity, 0, this.maxSanity());
    ...
    currentInteraction() {
        if (this.mode !== 'playing' || this.hp <= 0 || this.activeEnemy())
            return null;
        const here = key(this.player.x, this.player.y);
        if (key(this.layout.sanctuary.x, this.layout.sanctuary.y) === here && !this.sanctuaryUsed.has(this.floor)) {
        ...
        if (this.player.x === this.layout.exit.x && this.player.y === this.layout.exit.y) {
            if (!this.floorObjectiveReady())
                return { label: 'Inspect sealed way', hint: this.floorBlockedMessage() };
            if (!this.questFlags.has(`boss-${this.floor}-defeated`))
                return { label: `Confront ${ENEMY_DEFS[FLOOR_DATA[this.floor].boss].name}`, hint: 'A major encounter waits beyond the seal.' };
    ...
    currentObjective() {
        const data = FLOOR_DATA[this.floor];
        if (this.questFlags.has(`boss-${this.floor}-defeated`))
            return this.floor < 4 ? 'The guardian is dead. Return to the stair and descend.' : 'The final threshold is open. Decide what becomes of the abyss.';
        if (this.floor === 1 && this.questFlags.has('salt-seal'))
            return 'Take the Salt Seal to the far stair and face its guardian.';
        if (this.floor >= 2) {
            ...
            if (count < total)
                return `${data.objective} (${count}/${total} complete)`;
        }
    ...
    endingChoices() {
        if (this.mode !== 'ending' || this.endingTitle !== 'THE HEART WAITS')
            return [];
        const choices = [
    ...
    addJournalOnce(entry) {
        if (!this.journalEntries.includes(entry))
            this.journalEntries.push(entry);
    }
    saveGame(withLog) {
        if (!this.archetype || !this.layout || this.mode === 'title') {
            if (withLog)
                this.emit();
            return;
        }
        if (this.hp <= 0) {
            if (withLog) {
                this.logLines = [...this.logLines, 'A lost expedition cannot overwrite the last valid checkpoint.'].slice(-60);
                this.draw();
            }
            return;
        }
    ...
    loadGame() {
        ...
        if (!raw) {
            this.logLines = [...this.logLines, 'No readable local save exists yet, or browser storage is unavailable.'].slice(-60);
            this.emit();
            return;
        }
        try {
            ...
            if (!isSaveData(parsed))
                throw new Error('Unsupported or malformed save');
            const d = parsed;
            this.mode = d.mode;
            this.seed = d.seed;
            this.archetype = d.archetype;
            this.floor = d.floor;
            this.level = d.level;
            this.xp = d.xp;
            ...
            this.hp = d.hp;
            this.maxHp = d.maxHp;
            this.baseMaxSanity = d.baseMaxSanity;
            this.sanity = d.sanity;
            this.steps = d.steps;
            this.logLines = [...d.log, 'Local expedition restored.'].slice(-60);
            this.journalEntries = d.journal;
            this.inventory = d.inventory;
            this.equippedWeaponId = d.equippedWeaponId;
            this.equippedCharmId = d.equippedCharmId;
            this.enemies = d.enemies;
            this.loot = d.loot;
            this.triggeredEvents = new Set(d.triggeredEvents);
            this.questFlags = new Set(d.questFlags);
            this.sanctuaryUsed = new Set(d.sanctuaryUsed);
            this.discovered = new Set(d.discovered);
            this.activeEnemyId = d.activeEnemyId;
            this.itemSerial = d.itemSerial;
            this.turnSerial = d.turnSerial;
            this.abilityCooldown = d.abilityCooldown;
            this.guarded = d.guarded;
            this.counterReady = d.counterReady;
            this.endingTitle = d.endingTitle;
            this.endingText = d.endingText;
            this.setpieceTriggered = new Set(d.setpieceTriggered ?? []);
            this.secretCount = d.secretCount ?? 0;
            const savedAudio = d.audioEnabled ?? true;
            const savedMotion = d.reducedMotion ?? false;
            this.audioEnabled = savedAudio;
            this.reducedMotion = savedMotion;
            this.loadPreferences();
            ...
            this.logLines = [...this.logLines, 'The local save was unreadable or unsupported and has been discarded to prevent a broken resume loop.'].slice(-60);
            this.emit();
        }
    }
    log(message) {
    ...
    randomInt(min, max) { return min + Math.floor(this.roll() * (max - min + 1)); }
    sanityState() {
        if (this.sanity <= 0)
            return 'Beyond';
    ...
    gameFeel(kind) {
        if (this.reducedMotion)
            return;
        if (kind === 'critical') {
            this.cameras.main.shake(90, .012);
            this.cameras.main.flash(45, 210, 225, 210, false);
        }
        else if (kind === 'hurt')
            this.cameras.main.shake(120, .009);
        else if (kind === 'boss') {
            this.cameras.main.shake(260, .014);
            this.cameras.main.flash(80, 150, 175, 155, false);
        }
        else
    ...
    bossAttackText(enemy) {
        const phase = enemy.phase ?? 1;
        if (enemy.kind === 'salt_knight')
            return phase === 1 ? 'sweeps the bell-blade in a mineral arc' : 'shatters its own armor into a storm of salt';
        if (enemy.kind === 'choir')
            return phase === 1 ? 'releases a chord that buckles the floor' : 'splits into competing voices and screams both futures at once';
        if (enemy.kind === 'abyss_heart')
            return phase === 1 ? 'contracts and drags the corridor inward' : phase === 2 ? 'beats backwards, returning pain before the strike' : 'opens like a door and lets the outside look in';
        if (enemy.kind === 'void_saint')
            return phase === 1 ? 'turns gravity sideways with a gesture' : phase === 2 ? 'removes a second from the room and attacks inside the missing time' : 'breaks its halo into three black suns';
    ...
    enemyIntent(enemy) {
        ...
        if (!enemy.boss) {
            if (enemy.kind === 'warden' && (enemy.combatTurn + 1) % 3 === 0)
                return 'locks its salt shield and prepares a measured strike';
            if (enemy.kind === 'oracle')
                return 'predicts a thought you have not had yet';
            if (enemy.kind === 'mirror_stalker')
                return (enemy.combatTurn + 1) % 2 === 0 ? 'mirrors your breathing; the next contact will test sanity' : 'waits for you to choose first';
            if (enemy.kind === 'bell_devourer')
                return (enemy.combatTurn + 1) % 2 === 0 ? 'swallows every sound before a violent toll' : def.intent;
            return def.intent;
        }
        ...
        if (heavy)
            return this.bossAttackText(enemy);
        if (enemy.kind === 'void_saint' && phase >= 2)
            return 'moves its hand toward the place your body will occupy next';
    ...
    snapshot() {
        ...
        return {
            ...
            dead: this.hp <= 0, seed: this.seed, floor: this.floor, floorName: this.mode === 'title' ? 'The Drowned Observatory' : FLOOR_DATA[this.floor].name,
    ...
    emit() { this.game.events.emit('snapshot', this.snapshot()); }
    drawTitle() {
        this.specialText?.setVisible(false);
        this.g.clear();
        this.g.fillStyle(0x040604).fillRect(0, 0, 800, 500);
        for (let i = 0; i < 13; i++) {
            const inset = i * 27;
            this.g.lineStyle(1, 0x6f8d74, .12 + i * .008);
            this.g.strokeRect(105 + inset / 2, 60 + inset / 3, 590 - inset, 380 - inset / 1.5);
        }
        this.g.lineStyle(3, 0xa6bca9, .55);
        this.g.strokeCircle(400, 250, 92);
        this.g.strokeCircle(400, 250, 47);
    ...
    draw() {
        if (this.mode === 'title' || !this.layout) {
            this.drawTitle();
            this.emit();
            return;
        }
        ...
        for (let depth = visibleDepth; depth >= 1; depth--) {
            ...
            if (wall) {
                this.g.fillStyle(0x162019, .95);
                this.g.fillRect(nextInset, nextTop, w - nextInset * 2, h - nextTop * 2);
        ...
        this.drawCompass();
        if (this.activeEnemy())
            this.drawEnemy(this.activeEnemy().kind, this.activeEnemy().boss);
        else
            this.drawHallucinations(sanityRatio);
        this.drawLocalSpecialMarker();
        this.drawSetpieceAura();
        if (sanityRatio <= .2) {
        ...
        if (this.hp <= 0)
            this.drawOverlay('LOST TO THE ABYSS', 'Load your save or begin another expedition');
        else if (this.mode === 'ending')
            this.drawOverlay(this.endingTitle, this.endingTitle === 'THE HEART WAITS' ? 'Your final choice is shown in the expedition panel' : 'The expedition is complete');
        else
            this.clearOverlay();
        this.emit();
    }
    randomVisual(min, max, salt) {
        const r = seededRandom(`${this.seed}:visual:${this.steps}:${salt}`)();
        return min + Math.floor(r * (max - min + 1));
    }
    drawHallucinations(sanityRatio) {
        if (sanityRatio > .42)
            return;
        const seed = seededRandom(`${this.seed}:hallucination:${this.floor}:${this.steps}:${this.player.dir}`);
        if (seed() > (sanityRatio <= .2 ? .72 : .38))
            return;
    ...
    drawSideOpenings(depth, inset, top, bottom, nextInset) {
        const left = this.relativeCell(-1, depth), right = this.relativeCell(1, depth);
        this.g.lineStyle(2, 0x36523f, .65);
        if (left !== '#')
            this.g.strokeRect(inset, top + 18, Math.max(15, nextInset - inset), Math.max(20, bottom - top - 36));
        if (right !== '#')
            this.g.strokeRect(800 - nextInset, top + 18, Math.max(15, nextInset - inset), Math.max(20, bottom - top - 36));
    }
    relativeCell(side, forward) {
        const d = DIRS[this.player.dir], r = DIRS[((this.player.dir + 1) % 4)];
    ...
    drawExitMarker() {
        ...
        for (let depth = 1; depth <= 5; depth++) {
            ...
            if (this.layout.map[y]?.[x] === '#')
                break;
            if (x !== this.layout.exit.x || y !== this.layout.exit.y)
                continue;
            const size = Math.max(18, 80 - depth * 11);
            this.g.lineStyle(3, this.floorObjectiveReady() ? 0xc8d7aa : 0x715e54, .9);
            this.g.strokeRect(400 - size / 2, 250 - size, size, size * 1.7);
            break;
        }
    }
    drawLocalSpecialMarker() {
        if (this.activeEnemy()) {
            this.specialText?.setVisible(false);
            return;
        }
        ...
        if (key(this.layout.sanctuary.x, this.layout.sanctuary.y) === here && !this.sanctuaryUsed.has(this.floor))
            symbol = '✦ SURVEY LAMP';
        if (this.layout.npc && key(this.layout.npc.x, this.layout.npc.y) === here && !this.questFlags.has('cartographer-met'))
            symbol = '◇ MARA VEY';
        const altar = this.layout.altars.findIndex(p => key(p.x, p.y) === here);
        if (altar >= 0 && !this.questFlags.has(`altar-${this.floor}-${altar}`))
            symbol = this.floor === 2 ? '◉ HYMN ALTAR' : this.floor === 3 ? '✧ BLACK-STAR MEMORY' : '⌬ ANCHOR SIGIL';
        const sp = this.layout.setpieces?.findIndex(p => key(p.x, p.y) === here) ?? -1;
        if (sp >= 0)
            symbol = this.setpieceTriggered.has(`setpiece-${this.floor}-${sp}`) ? '◈ ANOMALOUS CHAMBER · RECORDED' : '◈ ANOMALOUS CHAMBER';
        if (!symbol) {
            this.specialText?.setVisible(false);
            return;
        }
    ...
    drawSetpieceAura() {
        ...
        if (index < 0 || this.activeEnemy())
            return;
        this.g.lineStyle(1, 0xa8c0ac, .16);
        for (let i = 0; i < 4; i++)
            this.g.strokeCircle(400, 248, 85 + i * 24);
    ...
    drawEnemy(kind, boss) {
        ...
        if (kind === 'leech' || kind === 'moth') {
            ...
            for (let i = -3; i <= 3; i++)
                this.g.strokeLineShape(new Phaser.Geom.Line(cx + i * 13, cy + 10, cx + i * 18, cy + 58));
        }
            ...
            if (boss) {
                this.g.lineStyle(8, 0xd8e2cf, .9);
                this.g.strokeLineShape(new Phaser.Geom.Line(cx + 60, cy - 80, cx + 100, cy + 100));
            }
        }
        else if (kind === 'bent' || kind === 'husk' || kind === 'mirror_stalker') {
            this.g.lineStyle(22, 0xc8d9c8, .9);
            this.g.strokeLineShape(new Phaser.Geom.Line(cx - 38, cy - 135, cx + 6, cy + 42));
            this.g.strokeCircle(cx - 46, cy - 145, 30);
            this.g.lineStyle(9, 0xc8d9c8, .9);
            this.g.strokeLineShape(new Phaser.Geom.Line(cx, cy + 30, cx - 42, cy + 115));
            this.g.strokeLineShape(new Phaser.Geom.Line(cx + 5, cy + 30, cx + 62, cy + 105));
        }
        else if (kind === 'bell_devourer') {
            this.g.fillEllipse(cx, cy - 15, 130, 180);
            this.g.fillStyle(0x070a08).fillEllipse(cx, cy + 10, 82, 100);
            this.g.lineStyle(7, 0xc8d9c8, .9);
            this.g.strokeCircle(cx, cy - 75, 42);
            this.g.strokeLineShape(new Phaser.Geom.Line(cx, cy - 35, cx, cy + 95));
        }
        else if (kind === 'choir') {
            this.g.fillEllipse(cx, cy - 10, 180, 210);
            this.g.fillStyle(0x070a08);
            for (let i = 0; i < 9; i++)
                this.g.fillEllipse(cx - 50 + (i % 3) * 50, cy - 80 + Math.floor(i / 3) * 65, 22, 11);
        }
        else if (kind === 'abyss_heart' || kind === 'void_saint') {
            this.g.lineStyle(kind === 'void_saint' ? 7 : 10, 0xd8e2cf, .92);
            this.g.strokeCircle(cx, cy - 5, kind === 'void_saint' ? 125 : 105);
            this.g.strokeCircle(cx, cy - 5, 58);
            ...
            this.g.fillEllipse(cx, cy - 45, 60, 85);
            this.g.fillStyle(0x070a08).fillCircle(cx - 11, cy - 52, 5).fillCircle(cx + 11, cy - 52, 5);
            this.g.lineStyle(8, 0xc8d9c8, .9);
            for (let i = -2; i <= 2; i++)
                this.g.strokeLineShape(new Phaser.Geom.Line(cx + i * 8, cy - 8, cx + i * 26, cy + 90));
        }
    }
    drawGlyphs(x, y, w, h) {
        ...
        for (let i = 0; i < 9; i++) {
            ...
            this.g.strokeCircle(gx, gy, 4 + (i % 3) * 3);
            this.g.strokeLineShape(new Phaser.Geom.Line(gx - 8, gy + 8, gx + 8, gy - 8));
        }
    }
    drawCrosshair() {
        this.g.lineStyle(1, 0xb4cdb8, .3);
        this.g.strokeLineShape(new Phaser.Geom.Line(390, 250, 410, 250));
        this.g.strokeLineShape(new Phaser.Geom.Line(400, 240, 400, 260));
    }
    drawMinimap() {
        ...
        for (let y = 0; y < this.layout.map.length; y++)
            for (let x = 0; x < this.layout.map[y].length; x++) {
                if (!this.discovered.has(key(x, y)))
                    continue;
                if (this.layout.map[y][x] === '.')
                    this.g.fillStyle(0x45634e, .55).fillRect(ox + x * scale, oy + y * scale, 4, 4);
            }
        if (lie) {
        ...
        const marker = (p, color, size = 3) => {
            if (!p || !this.discovered.has(key(p.x, p.y)))
                return;
        ...
        if (!this.sanctuaryUsed.has(this.floor))
            marker(this.layout.sanctuary, 0x8bb8a2, 3);
        if (this.layout.npc && !this.questFlags.has('cartographer-met'))
            marker(this.layout.npc, 0x9cb0d2, 3);
        this.layout.altars.forEach((p, i) => { if (!this.questFlags.has(`altar-${this.floor}-${i}`))
            marker(p, 0xb295ba, 3); });
    ...
    surveyBearing() {
        ...
        let targets = [];
        if (this.questFlags.has(`boss-${this.floor}-defeated`))
            targets = [this.layout.exit];
        else if (this.floor === 1 && !this.questFlags.has('salt-seal') && this.layout.miniboss)
            targets = [this.layout.miniboss];
        else if (this.floor >= 2 && !this.floorObjectiveReady())
            targets = this.layout.altars.filter((_, i) => !this.questFlags.has(`altar-${this.floor}-${i}`));
        else
            targets = [this.layout.exit];
        if (!targets.length)
            return '';
        ...
        for (let dir = 0; dir < DIRS.length; dir++) {
            ...
            if (nextDistance === best.distance - 1)
                return ` · SURVEY ${labels[dir]} ${best.distance}`;
        }
        return ` · SURVEY ? ${best.distance}`;
    }
    drawCompass() {
        const labels = ['N', 'E', 'S', 'W'];
        this.compass.setText(`${labels[this.player.dir]} · ${this.player.x},${this.player.y} · ${FLOOR_DATA[this.floor].name}${this.surveyBearing()}`);
    }
    drawOverlay(title, subtitle) {
        ...
        if (!this.overlayText) {
            ...
            this.overlaySubtext?.setText(subtitle).setVisible(true);
        }
    }
    clearOverlay() { this.overlayText?.setVisible(false); this.overlaySubtext?.setVisible(false); }
}
