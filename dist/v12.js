...
function renderTools(){
 const p=totalProgress();const badge=document.getElementById('badge');if(badge&&!window.__abyssalV14&&!window.__abyssalV13)badge.textContent='FINAL · v1.2.0'
...
async function importFile(file){
 ...
 const previous=safeGet(SAVE_KEY),previousCodex=structuredClone(codex),previousLanguage=currentLang();if(!safeSet(SAVE_KEY,parsed.saveText)){showToast(t('storageFail'),'error');return}
 try{
  ...
  codex=parsed.codex;if(!saveCodex(codex))throw new Error('codex persistence failed');if(parsed.language)api.setLanguage(parsed.language);showToast(t('imported'));renderTools()
 }catch{
  if(previous===null)safeRemove(SAVE_KEY);else safeSet(SAVE_KEY,previous);codex=previousCodex;saveCodex(codex);api.setLanguage(previousLanguage);if(previous===null)api.send('title');else api.send('load');showToast(t('invalid'),'error');renderTools()
 }
}
