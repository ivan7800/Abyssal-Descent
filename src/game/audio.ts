export type AudioCue = 'step' | 'turn' | 'pickup' | 'interact' | 'attack' | 'critical' | 'hurt' | 'sanity' | 'boss' | 'victory' | 'rest' | 'secret'
...
export class AbyssAudio {
  ...
  setEnabled(enabled: boolean) {
    ...
    if (!enabled) this.stopDrone()
  }

  startDrone(floor: number) {
  ...
  stopDrone() {
    ...
    try { this.drone?.disconnect(); this.droneGain?.disconnect() } catch { /* already disconnected */ }
    this.drone = null; this.droneGain = null
  }

  cue(cue: AudioCue, intensity = 1) {
    const ctx = this.ensure(); if (!ctx || !this.master) return
    ...
    const spec: Record<AudioCue, [number, number, number]> = {
    ...
    if (cue === 'critical' || cue === 'victory' || cue === 'secret') osc.frequency.exponentialRampToValueAtTime(freq * 1.7, now + duration)
